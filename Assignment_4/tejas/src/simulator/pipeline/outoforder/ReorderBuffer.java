...
public class ReorderBuffer extends SimulationElement{
	...
	int regRootInconsequentialCount = 0;
	int[] registerStatus = new int[Registers.getMaxIntegerRegisters()];
	...
	{	
		...
		for(int no_insts = 0; no_insts < retireWidth; no_insts++)
		{
			...
			if(first.isWriteBackDone() == true)
			{
				...
				// Set the status of the source registers to 0
				if(firstInstruction.getSourceOperand1().isIntegerRegisterOperand())
				{
					registerStatus[(int)firstInstruction.getSourceOperand1().getValue()] = 0;
				}
				if(firstInstruction.getSourceOperand2().isIntegerRegisterOperand())
				{
					registerStatus[(int)firstInstruction.getSourceOperand2().getValue()] = 0;
				}

				// Check if the status of the destination register is 1
				if(firstInstruction.getDestinationOperand().isIntegerRegisterOperand())
				{
					if(registerStatus[(int)firstInstruction.getDestinationOperand().getValue()] == 1)
					{
						regRootInconsequentialCount++;
						System.out.println("regRootInconsequentialCount : " + regRootInconsequentialCount);
					}

					registerStatus[(int)firstInstruction.getDestinationOperand().getValue()] = 1;
				}
